    ...
    Self: Send + Sync,
{
    ...
    fn initialize(&mut self, engine: &EngineInterface) -> SafeResult<(), SafeError>;
    ...
    fn begin_play(&mut self, engine: &EngineInterface) -> SafeResult<(), SafeError>;

    /// Called every frame while the game is playing.
    fn update(&mut self, engine: &EngineInterface, delta: f32) -> SafeResult<(), SafeError>;
    ...
    fn end_play(&mut self, engine: &EngineInterface) -> SafeResult<(), SafeError>;
